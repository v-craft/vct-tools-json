...
M_TEST(Value, Bool) {
    ...
    M_ASSERT_EQ( copy_init_true.to<json::Bool>(), true );
    M_ASSERT_EQ( copy_init_false.to<json::Bool>(), false );
    M_ASSERT_EQ( copy_init_wrapper_true.to<json::Bool>(), true );
    M_ASSERT_EQ( copy_init_wrapper_false.to<json::Bool>(), false );
    ...
    // Test get<T>() method
    M_ASSERT_EQ( true_val.to<json::Bool>(), true );
    M_ASSERT_EQ( false_val.to<json::Bool>(), false );
    
    // Test get_ref<T>() method
    M_ASSERT_NO_THROW( auto& ref = true_val.get<json::Bool>() );
    ...
    M_ASSERT_NO_THROW( auto& ref_true = ref_test_true.get<json::Bool>() );
    M_ASSERT_NO_THROW( auto& ref_false = ref_test_false.get<json::Bool>() );
    
    M_ASSERT_EQ( ref_test_true.get<json::Bool>(), true );
    M_ASSERT_EQ( ref_test_false.get<json::Bool>(), false );
    
    // Test const get_ref
    const json::Value const_bool{true};
    M_ASSERT_NO_THROW( auto& const_ref = const_bool.get<json::Bool>() );
    ...
    M_ASSERT_THROW( std::ignore = wrong_type_val.get<json::Bool>(), std::runtime_error );
    
    json::Value num_val_for_ref{42.0};
    M_ASSERT_THROW( std::ignore = num_val_for_ref.get<json::Bool>(), std::runtime_error );
    
    json::Value null_val_for_ref{nullptr};
    M_ASSERT_THROW( std::ignore = null_val_for_ref.get<json::Bool>(), std::runtime_error );
    ...
    M_ASSERT_EQ( implicit_assign.to<json::Bool>(), true );
    
    implicit_assign = false;
    M_ASSERT_EQ( implicit_assign.to<json::Bool>(), false );
    ...
    M_ASSERT_NO_THROW( auto num_from_true = true_val.to<json::Number>() );
    M_ASSERT_NO_THROW( auto num_from_false = false_val.to<json::Number>() );
    M_ASSERT_EQ( true_val.to<json::Number>(), 1.0 );  // true -> 1.0
    M_ASSERT_EQ( false_val.to<json::Number>(), 0.0 ); // false -> 0.0
    
    // Test Bool to other numeric types (should work due to arithmetic conversion)
    M_ASSERT_NO_THROW( auto int_from_true = true_val.to<int>() );
    M_ASSERT_NO_THROW( auto int_from_false = false_val.to<int>() );
    M_ASSERT_EQ( true_val.to<int>(), 1 );   // true -> 1
    M_ASSERT_EQ( false_val.to<int>(), 0 );  // false -> 0
    
    M_ASSERT_NO_THROW( auto float_from_true = true_val.to<float>() );
    M_ASSERT_NO_THROW( auto double_from_false = false_val.to<double>() );
    M_ASSERT_EQ( true_val.to<float>(), 1.0f );  // true -> 1.0f
    M_ASSERT_EQ( false_val.to<double>(), 0.0 ); // false -> 0.0
    ...
    M_ASSERT_NO_THROW( auto num_val = bool_to_num_true.to<json::Number>() );
    M_ASSERT_EQ( bool_to_num_true.to<json::Number>(), 1.0 );  // true -> 1.0
    M_ASSERT_EQ( bool_to_num_false.to<json::Number>(), 0.0 ); // false -> 0.0
    
    // Test boolean logic consistency
    json::Value logical_true{true};
    json::Value logical_false{false};
    
    M_ASSERT_TRUE( logical_true.to<json::Bool>() );
    M_ASSERT_FALSE( logical_false.to<json::Bool>() );
    
    // Test that boolean values are exactly true/false
    M_ASSERT_TRUE( logical_true.to<json::Bool>() == true );
    M_ASSERT_TRUE( logical_false.to<json::Bool>() == false );
    M_ASSERT_FALSE( logical_true.to<json::Bool>() == false );
    M_ASSERT_FALSE( logical_false.to<json::Bool>() == true );
}
