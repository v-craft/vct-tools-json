...
M_TEST( README, readme ) {
    ...
    M_EXPECT_EQ(val1[1].to<bool>(), 0); // 输出 0 （没有指定 boolalpha）
    ...
    M_EXPECT_TRUE( (val_arr_1 == val_arr_2) ); // true
    M_EXPECT_FALSE( (val_arr_1 == val_arr_3) ); // false


    {
        json::Value v_null;
        MyData d_null{ v_null }; // 什么都没有，因此全部字段都是默认值
        M_EXPECT_TRUE( d_null.active ); // true，因为构造函数指定了默认值为 true
        ...
        M_EXPECT_TRUE(  d_object.m_name == "Test User" ); // true

        json::Value v_data{ d_object }; // 将 MyData 转换为 JSON 对象
        M_EXPECT_TRUE(  v_data["id"] == 42 ); // true
    }
